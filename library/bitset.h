...
{
public:
	using value_type = typename std::conditional_t<std::is_enum_v<T>, enum_type_base<T>, integral_type_base<T>>::type;
	...
	constexpr bool operator[](value_type position) const
	{
		return has(position);
	}

	constexpr bitset& set(value_type position, bool value = true)
	{
	...
	constexpr bitset& reset(value_type position)
	{
		const value_type pos = static_cast<value_type>(position);
		ASSERTION(pos <= Bits && "Position supplied exceeded the number of bits!");
	...
	constexpr bool has(value_type position) const
	{
		const value_type pos = static_cast<value_type>(position);
		ASSERTION(pos <= Bits && "Position supplied exceeded the number of bits!");
	...
	constexpr bool test(value_type position) const
	{
		return has(position);
	}
	...
	constexpr bitset& flip(value_type position)
	{
		const value_type pos = static_cast<value_type>(position);
		ASSERTION(pos <= Bits && "Position supplied exceeded the number of bits!");
