...
public:
	...
	{
	public:
		constexpr deleter(linear_allocator& allocator) :
			allocator{ allocator }
		{}
	...
	*/
	...
	void* allocate_memory(allocate_info const& info)
	{
		...
		ASSERTION(((m_offset + sizeof(memory_headeer) + info.size + info.alignment) < m_size) && 
	...
	template <typename T>
	T* allocate_memory(allocate_type_info<T> const& info)
	{
		return static_cast<T*>(allocate_memory({ .size = info.count = sizeof(T), .alignment = info.alignment, .location = info.location }));
	}

	template <typename T, typename... Args>
	T* allocate_memory(allocate_type_info<T> const& info, Args&&... args)
	{
		T* ptr = allocate_memory(info);
