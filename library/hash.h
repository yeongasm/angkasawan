...
template <typename traits, typename hasher, std::derived_from<container_growth_policy> growth_policy, provides_memory provided_allocator = system_memory, typename probe_distance_limit = probe_sequence_length_limit<4096>>
...
public:
	...
	struct hash_container_const_iterator
	{
		...
		constexpr hash_container_const_iterator(bucket_info* data, hash_container_base* hash_map) :
		...
		constexpr bucket_value_type bucket() const { return static_cast<bucket_value_type>(info - hash->m_info) ;}
		...
		bucket_info* info;
		hash_container_base* hash;
	};

	struct hash_container_iterator : public hash_container_const_iterator
	{
		...
		constexpr reference	operator* () const { return *(bucket->data); }
		constexpr pointer	operator->() const { return bucket->data; }
	};

	using iterator			= hash_container_iterator;
	using const_iterator	= hash_container_const_iterator;

	constexpr hash_container_base() requires std::same_as<allocator_type, system_memory> :
		m_metadata{}, m_data{}, m_len{}, m_capacity{}, m_max_load_factor{ 0.75f },
	{}

	constexpr hash_container_base(allocator_type& allocator) requires !std::same_as<allocator_type, system_memory> :
	...
	constexpr hash_container_base(size_t length) requires std::same_as<allocator_type, system_memory> :
		hash_container_base{}
	{
		reserve(length);
	}

	constexpr hash_container_base(size_t length, allocator_type& allocator) requires !std::same_as<allocator_type, system_memory> :
		hash_container_base{ allocator }
	{
		reserve(length);
	}

	constexpr hash_container_base(hash_container_base const& rhs) requires std::same_as<allocator_type, system_memory> :
		m_metadata{}, m_data{}, m_len{}, m_capacity{}, m_max_load_factor{ 0.75f }
	{
		*this = rhs;
	}
	...
	constexpr void release()
	{
		...
		if (m_capacity)
		{
			allocator_interface::free_storage(m_metadata);
	...
	constexpr float32	load_factor	() const { return m_len / m_capacity; }
...
protected:
	...
	constexpr void destruct(size_t from, size_t to)
	{
		...
		{
			m_data[i].~type();
			m_metadata->p_bucket_info[i].~bucket_info();
		}
	}
	...
	constexpr void reserve(size_t capacity)
	{
		...
		uint8* pBuffer = static_cast<uint8*>(allocator_interface::allocate_storage({ .size = metadataCapacity, .location = std::source_location::current() }));
		...
		m_data = allocator_interface::allocate_storage<type>({ .count = capacity, .location = std::source_location::current() });
		...
		if (size())
		{
			...
			allocator_interface::free_storage(metadataOld);
			allocator_interface::free_storage(dataOld);
		}
	}
...
private:
	...
	{
		m_metadata->grow_on_next_inser ^= true;
	}
};
