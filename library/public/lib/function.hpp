...
struct function_trait<Ret(Args...)>
{
	...
	struct operator_impl
	{
		...
		auto operator()(Arguments&&... args) -> return_type requires (... && std::is_convertible_v<Arguments, Args>)
		{
		...
		auto operator()(fn::detail::forward_arg<Args>... args) -> return_type
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
			...
			return fn.m_vtbl.invocable(fn.m_storage.accessor, std::forward<fn::detail::forward_arg<Args>>(args)...);
		}
	};
...
struct function_trait<Ret(Args...) const>
{
	...
	{
		template <typename... Arguments>
		auto operator()(Arguments&&... args) const -> return_type requires (... && std::is_convertible_v<Arguments, Args>)
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
		...
		auto operator()(fn::detail::forward_arg<Args>... args) const -> return_type
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
...
struct function_trait<Ret(Args...) noexcept>
{
	...
	{
		template <typename... Arguments>
		auto operator()(Arguments&&... args) noexcept -> return_type requires (... && std::is_convertible_v<Arguments, Args>)
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
		...
		auto operator()(fn::detail::forward_arg<Args>... args) noexcept -> return_type
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
...
struct function_trait<Ret(Args...) const noexcept>
{
	...
	{
		template <typename... Arguments>
		auto operator()(Arguments&&... args) const noexcept -> return_type requires (... && std::is_convertible_v<Arguments, Args>)
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
		...
		auto operator()(fn::detail::forward_arg<Args>... args) const noexcept -> return_type
		{
			auto&& fn = *(static_cast<FnType*>(this));
			if (!fn.m_vtbl.invocable)
			{
...
public:
	...
	constexpr function(Functor&& functor) requires (!requires { typename Functor::internal_disable_constructor_on_move; }) :
		...
		m_vtbl{}
	{
		using box_type = box<Functor>;
		constexpr bool emplace_insitu = sizeof(box_type) <= properties.capacity;

		vtable_t::template construct<emplace_insitu>(m_storage.accessor, make_box(std::forward<Functor>(functor)));
