...
auto MeshSbfPack::MeshPackIterator::operator++() noexcept -> MeshPackIterator&
{
    ...
    auto const& header = *reinterpret_cast<MeshViewHeader*>(m_cursor);

    if (header.descriptor.tag != 'VM')
    {
    ...
    m_cursor += sizeof(MeshViewHeader) + header.descriptor.sizeBytes;
...
auto MeshSbfPack::from_memory(std::span<std::byte> blob) -> bool
{
    ...
    auto const& meshViewGroup = *reinterpret_cast<MeshViewGroupHeader*>(&blob[offset]);

    if (meshViewGroup.descriptor.tag != 'GVM' || 
    ...
    offset += sizeof(MeshViewGroupHeader);
...
auto MeshSbfPack::from_mmap_file(core::sbf::File const& mmapFile) -> bool
{
	...
	auto const& meshViewGroup = *reinterpret_cast<MeshViewGroupHeader*>(ptr);
	...
	ptr += sizeof(MeshViewGroupHeader);
