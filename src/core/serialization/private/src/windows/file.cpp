...
File::File(FileMMapInfo const& info)
{
	open(info);
}
...
auto File::open(std::filesystem::path const& path, bool map) -> bool
{
	...
	return (map) ? this->map() : m_handle.file != INVALID_HANDLE_VALUE;
}

auto File::open(FileMMapInfo const& info) -> bool
{
	DWORD const shareMode = [](FileMMapInfo const& info) -> DWORD
	{
		DWORD shareMode{};

		using enum FileShareMode;

		shareMode |= ((info.shareMode & Shared_Delete)	!= None) * FILE_SHARE_DELETE;
		shareMode |= ((info.shareMode & Shared_Read)	!= None) * FILE_SHARE_READ;
		shareMode |= ((info.shareMode & Shared_Write)	!= None) * FILE_SHARE_WRITE;

		return shareMode;
	}(info);

	DWORD const createDisposition = [](FileMMapInfo const& info) -> DWORD
	{
		using enum FileCreateDisposition;

		switch (info.createDisposition)
		{
		case Create_Always:
	...
	m_handle.file = CreateFileW(info.path.c_str(), GENERIC_READ, shareMode, nullptr, createDisposition, FILE_ATTRIBUTE_READONLY, nullptr);

	return (info.map) ? this->map() : m_handle.file != INVALID_HANDLE_VALUE;
}
