...
auto ShaderCompiler::compile_shader(ShaderCompileInfo const& info) -> CompilationResult
{
	...
	auto unoptimized = compiler.CompileGlslToSpv(info.sourceCode.data(), type, info.path.data(), options);
	...
	auto optimized = compiler.CompileGlslToSpv(info.sourceCode.data(), type, info.path.data(), options);
